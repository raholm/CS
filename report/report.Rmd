...
posterior <- function(y,meanS,index,sigmasq){ 
  
  if (index == 1){
   return( rnorm(1, mean = (y[index] + meanS[index + 1])/2, sd = sqrt(sigmasq))  ) 
  }  
  
  if (index == length(meanS)){
   return( rnorm(1, mean = (y[index] + meanS[index - 1])/2, sd = sqrt(sigmasq)) *posterior(y,meanS,index- 1, sigmasq) )
  } 
  
  #If not any of above statements
  return( rnorm(1, mean = (y[index] + meanS[index - 1] + meanS[index + 1])/3, 
                sd = sqrt(3/(2*sigmasq)) ) * posterior(y,meanS, index-1,sigmasq) )
...
gibbs <- function(data, tmax){
  ...
  result <- matrix(0,nrow = tmax, ncol = d) 
  
  for (i in 1:tmax){
    for (j in 1:d){
      
      result[i,j] <- posterior(data, index = j, mean = xo, sigmasq = 0.2)
      xo <- result[(i-1):(i+1),j]
...
```{r}
mu <- gibbs(data = chem$Y, tmax = 1000)
