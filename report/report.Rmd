...
```{r}
hist(npboot$t, xlim = c(-1,0.7), breaks = 50, main = "Histogram for bootstrap t-values", xlab ="T")
...
set.seed(123456)
statperm<-teststat_permutation(data, B, teststat(loess))
hist(statperm,breaks = 40)
pvalue<-sum(abs(statperm) >= abs(teststat(model = loess)(data))) / B
```
