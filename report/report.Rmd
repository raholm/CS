...
## abline(v=quantile(npboot$t, c(0.95)), col = "red", lwd = 2)
## abline(v=teststat(loess)(data), col="orange", lwd=2)
...
The mean price of the data set is `r mean(price$Price)` and we think the histogram above looks like a Gamma distribution.
...
\begin{align*}
\widehat{Var \left[ T( \cdot ) \right] } &= \frac{1}{B - 1} \sum_{i=1}^{B} \left( T(D_{i}^{*}) - \overline{T(D^{*})} \right)^2.
\end{align*}
...
The bias corrected estimate of the mean is `r bias_corrected_estmean`, the bias correction, $\sum_{i=1}^{B} \left( T(D_{i}^*) - T(D) \right)$, is `r bias_correction`, and the variance is estimated to `r estvar`.
...
\begin{align*}
\widehat{Var \left[ T( \cdot ) \right] } &= \frac{1}{n (n- 1)} \sum_{i=1}^{n} \left( T_{i}^{*} - J(T) \right)^2 \\
\intertext{where,}
T_{i}^{*} &= nT(D) - (n - 1) T(D_{i}^{i}), \\
J(T) &= \frac{1}{n} \sum_{i=1}^{n} T_{i}^{*}.
\end{align*}
...
We get the estimate of mean to be `r jackestmean` and variance estimate to `r jackvar` which is similar to what we found previously with the bootstrap method.
